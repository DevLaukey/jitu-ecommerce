...
const ImageModal = ({ showModal, setShowModal }) => {
  ...
  const onDrop = useCallback((acceptedFiles) => {
    console.log(object);(acceptedFiles);
    setImageAdded(true);
  }, []);
  const { getRootProps, getInputProps, isDragActive } = useDropzone({
    ...
    useEffect(() => {
      // let rows, total = 0;
      axios.post(`${baseURL}/profile-upload`,imageRemoved).then((response) => {
                    ...
                    {imageAdded ? (
                      <div class="block">
                        ...
                        <p class="mb-2 text-sm text-gray-500 dark:text-gray-400">
                                                  Change the image? <span onClick={() => {
                                                      setImageAdded(false);
                          }} className="text-blue-400">Here</span>
                        </p>
                      </div>
                    ) : (
                      <label
